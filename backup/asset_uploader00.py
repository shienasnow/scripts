...
class AssetUpload(QWidget):
    ...
    def make_ui(self):
        ...
        ui_file_path = path + "/asset_uploader.ui"
    ...
    def event_func(self):
        ...
        self.ui.pushButton_upload.clicked.connect(self.sg_upload_thumbnail)
        self.ui.pushButton_upload.clicked.connect(self.sg_upload_mov)
        self.ui.pushButton_upload.clicked.connect(self.sg_upload_comment)
...
# # 이거 함수로 따로 빼서 가져오지 말고 그냥 생성자에서 self로 받기
...
# Backend : upload 버튼 누르면 실행
    def sg_status_update(self):
        ...
        print(f"Asset 엔티티에서 {task_id}의 status를 pub으로 업데이트합니다.") # publish에서는 fin으로 바꾸기

    def sg_upload_thumbnail(self):
        print("thumbnail 이미지를 업로드합니다.")
        png_path = self.capture_path
        asset_id = self.selected_asset_id
        self.sg.upload("Asset", asset_id, png_path, "image")

    def sg_upload_mov(self):
        print("mov를 version에 업로드하고 링크합니다.")


        project_id = "" ############################################# project 이름으로 project id 구하기
        user_id = "" ################################################ artist 이름으로 artist id 구하기
        version_code = "" ########################################### wip 폴더에서 version 정보 따와서 str으로 넣기
        ...
        self.sg.update("Version", version_id, {"user" : {"type" : "HumanUser", "id" : user_id}}) # artist
    ...
    def sg_upload_comment(self):
        comment = self.ui.plainTextEdit_comment.text()
        print(f"올릴 코멘트 내용 확인 :{comment}")

        self.sg.update("Asset", self.selected_asset_id, {"description": comment}) # asset description에 작업자 코멘트 업로드
